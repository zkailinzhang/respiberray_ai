...
def DecBolt():
    try:
        ...
        while (cap.isOpened() ):
            ...
            names_path = './coco.bolt.names'
            model_path = './yolov5s.bolt.onnx'
            bolt_frame,bbox = Detect(raw_frame,names_path,model_path)
            time_now = (datetime.datetime.now()).strftime(f'%Y-%m-%d %H:%M:%S')

            cv2.imwrite("frame_bolt.jpg",bolt_frame)
            filei = open("frame_bolt.jpg",'rb')
            dataimg = filei.read()

            sql_bj = "INSERT INTO diagnosis_history(task_name,fault_id,image,time_start) values(%s,%s,%s,%s);"
            cursor.execute(sql_bj,('螺栓检测',0,dataimg,time_now))
            db.commit()

            anglelist = luosuandet(bolt_frame)
            if len(bbox)!=0:
                time_now = (datetime.datetime.now()).strftime(f'%Y-%m-%d %H:%M:%S')

                sql_bj = "INSERT INTO diagnosis_list(task_name,fault_id,image,time_start) values(%s,%s,%s,%s);"
                
                if abs(anglelist[0] -anglelist[1]) >luoshuan_thresh:
                    cursor.execute(sql_bj,('螺栓松动',1,dataimg,time_now))
                else:
                    cursor.execute(sql_bj,('螺栓正常',1,dataimg,time_now))
